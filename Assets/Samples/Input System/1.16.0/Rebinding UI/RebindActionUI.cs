...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            string oldOverriddenPath = newBinding.effectivePath;
            ...
            foreach(InputAction otherAction in action.actionMap.actions)
            {
                if(otherAction == action)
                    continue; // Don't compare against bindings on the same action

                for(int i = 0; i < otherAction.bindings.Count; i++)
                {
                    InputBinding otherBinding = otherAction.bindings[i];
                    if(otherBinding.overridePath == oldOverriddenPath){
                        otherAction.ApplyBindingOverride(i, oldOverriddenPath);
                    }
                }
            }
        }
            ...
            foreach(InputBinding binding in action.actionMap.bindings)
            {
                if(binding.action == newBinding.action)
                    ...
                    if(binding.effectivePath == newBinding.effectivePath){
                        Debug.LogWarning($"Duplicate binding detected: '{newBinding.effectivePath}' is already bound");
                        return true;
                    }
            }
        ...
        [Tooltip("Whether to override the action label with the name of the action. If false, the action label is expected to be set up manually and will not be changed by the component.")]
        [SerializeField]
